...
When looking up an IP address, the `response` includes a `country_name` which includes the country name based on American English and `is_eu` which returns `true` if the country is a member of the European Union (EU). 
```rust 
...
```rust
let config = IpInfoConfig { country_file_path: Some("custom.json".to_string()), eu_file_path: Some("custom_eu.json".to_string()), ..Default::default() };
```
